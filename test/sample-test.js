...
describe("Volcano Coin", () => {
  ...
  beforeEach(async () => {
    ...
    volcanoContract = await Volcano.deploy(100);
...
//     await expectRevert(
//       volcanoContract.transferToken(constants.ZERO_ADDRESS, 10),
...
//   });
  ...
  it("has 18 decimals", async () => {
      let initialValue = await volcanoContract.decimals();
      expect(initialValue).to.equal(18)
  });

  it("assigns initial balance", async () => {
      let expectedBalance = 100;
      let actualBalance = await volcanoContract.totalSupply()
